    ...
    async (scoutName: string): Promise<{ scoutId: string; offer: string }> => {
      ...
      dataChannel.onopen = () => {
        ...
        if (connectedScout) {
          ...
          updateConnectedScouts();
          console.log(
            `✅ Scout ${scoutName} data channel opened (existing connected entry updated)`
          );
        }
      };
      ...
      setTimeout(() => {
        ...
        if (
          scout.connection.connectionState === 'new' ||
          scout.connection.connectionState === 'connecting'
        ) {
          console.warn(
            `⏱️ Pending scout ${scout.name} timed out, cleaning up stale pending connection`
          );
          try {
  ...
  const processScoutAnswer = useCallback(
    async (scoutId: string, answerString: string): Promise<void> => {
      ...
      for (const candidateKey of candidateKeys) {
        ...
        console.log(
          `📦 Context: Processing ${bufferedCandidates.length} buffered lead ICE candidates for ${scout.name}`
        );
        ...
        try {
          ...
          if (mode === 'lead' && message.type === 'join' && message.role === 'scout') {
            ...
            if (existingPending) {
              ...
              if (!stalePending) {
                console.log(
                  '👑 Context: Scout already has active pending connection, ignoring duplicate join'
                );
                return;
              }

              console.warn(
                `♻️ Context: Replacing stale pending scout connection for ${message.peerName || message.peerId}`
              );
              try {
            ...
            if (existingConnected) {
              ...
              const canReplace =
                existingConnected.status === 'disconnected' ||
                state === 'disconnected' ||
                state === 'failed' ||
                state === 'closed';
              if (!canReplace) {
              ...
              console.warn(
                `♻️ Context: Replacing stale connected scout entry for ${existingConnected.name}`
              );
              try {
              ...
              connectedScoutsRef.current = connectedScoutsRef.current.filter(
                s => s !== existingConnected
              );
              updateConnectedScouts();
            }
              ...
              if (mode === 'lead') {
                ...
                if (scout?.connection) {
                  ...
                  if (scout.connection.remoteDescription) {
                    ...
                    console.log(
                      `📦 Context: Buffered lead ICE candidate for ${message.peerId} (${pendingIceCandidatesRef.current.get(bufferKey)!.length} queued)`
                    );
                  }
                } else {
                  console.warn(
