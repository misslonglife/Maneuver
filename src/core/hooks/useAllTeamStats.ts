...
export const useAllTeamStats = (eventKey?: string): UseAllTeamStatsResult => {
  ...
  const scoutedTeamStats = useMemo(() => {
    ...
    const coprEventKeys = eventKey ? [eventKey] : [...new Set(getCachedCOPREventKeys())];

    const coprByEvent = new Map(coprEventKeys.map(key => [key, getCachedEventCOPRs(key)] as const));
  ...
  useEffect(() => {
    ...
    const loadCachedOnlyStats = async () => {
      ...
      try {
        ...
        if (!cancelled) {
          supplemental.sort(
            (a, b) => a.teamNumber - b.teamNumber || a.eventKey.localeCompare(b.eventKey)
          );
