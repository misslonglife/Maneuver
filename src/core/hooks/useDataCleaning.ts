...
export const useDataCleaning = (
    ...
    try {
      ...
      const filtered = parsed.filter(item => {
        ...
        if (
          ...
          return (
            (item as { eventKey: string }).eventKey.trim().toLowerCase() !== eventKey.toLowerCase()
          );
        }
  ...
  const clearEventLocalStorageData = useCallback(
    (eventKey: string) => {
      ...
      if (
        currentEventKey &&
        currentEventKey.trim().toLowerCase() === normalizedEventKey.toLowerCase()
      ) {
  ...
  const handleClearEventData = useCallback(
    async (eventKey: string) => {
      ...
      try {
        const scoutingCollection = db.scoutingData
          .where('eventKey')
          .equalsIgnoreCase(normalizedEventKey);
        const pitCollection = pitDB.pitScoutingData
          .where('eventKey')
          .equalsIgnoreCase(normalizedEventKey);
        const predictionCollection = gameDB.predictions
          .where('eventKey')
          .equalsIgnoreCase(normalizedEventKey);
