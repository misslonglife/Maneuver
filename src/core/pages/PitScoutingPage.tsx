  ...
  const refreshAssignedTeams = useCallback(() => {
    ...
    if (!eventKey && scoutName) {
      ...
      for (let i = 0; i < localStorage.length; i++) {
        ...
        if (
          key?.startsWith('pit_assignments_') &&
          !key.startsWith('pit_assignments_meta_') &&
          !key.startsWith('pit_assignments_mine_')
        ) {
          candidateEvents.push(key.replace('pit_assignments_', ''));
        }
      }

      const eventWithAssignments = candidateEvents.find(
        candidateEventKey => loadMyPitAssignments(candidateEventKey, scoutName).length > 0
      );
    ...
    const myAssignments = loadMyPitAssignments(eventKey, scoutName).sort(
      (a, b) => Number(a.completed) - Number(b.completed) || a.teamNumber - b.teamNumber
    );
              ...
              {assignedTeams.length === 0 ? (
                ...
                <div className="space-y-2">
                  {assignedTeams.map(assignment => {
                    const isCurrentTeam =
                      typeof formState.teamNumber === 'number' &&
                      formState.teamNumber === assignment.teamNumber;
                    ...
                    return (
                      <button
                        ...
                        className={`w-full rounded-md border px-3 py-2 text-left transition-colors ${
                          isCurrentTeam ? 'border-primary bg-primary/10' : 'hover:bg-muted/50'
                        }`}
                        onClick={() => {
