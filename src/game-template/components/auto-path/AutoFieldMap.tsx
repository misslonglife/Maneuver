...
function AutoFieldMapContent({
  ...
  const firstConfirmedStartTimestamp = useMemo(() => {
    ...
    return typeof firstStartAction?.timestamp === 'number' ? firstStartAction.timestamp : null;
  }, [actions]);
  const autoCueTimerStartTimestamp = startAutoCueFromAutoScreenEntry
    ? autoScreenEnteredAtRef.current
    : startAutoCueFromStartConfirmation || autoAdvanceToTeleopAfter20s
      ? firstConfirmedStartTimestamp
      : null;
  const autoCueCountdownSeconds =
    autoCueTimerStartTimestamp === null
      ? null
      : Math.max(0, Math.ceil((AUTO_CUE_TARGET_MS - autoElapsedMs) / 1000));
  const shouldPulseAutoBorder = autoElapsedMs >= AUTO_CUE_TARGET_MS;
  const shouldAutoAdvanceToTeleop =
    autoAdvanceToTeleopAfter20s && elapsedSinceStartConfirmationMs >= AUTO_CUE_TARGET_MS;
  ...
  const handleElementClick = useCallback(
    (elementKey: string) => {
      ...
      if (
        ...
        isSelectingCollect ||
        selectedStartKey ||
        (!clearedPersistentStuck && isAnyStuck) ||
        isBrokenDown
      ) {
        return;
      }
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      const canPassFromZone =
        visibleAutoElementSet.has('pass') || visibleAutoElementSet.has('pass_alliance');
      const canCollectFromZone =
        visibleAutoElementSet.has('collect_alliance') ||
        visibleAutoElementSet.has('collect_neutral');
          ...
          {actions.length === 0 && !selectedStartKey && !pendingShotTypeWaypoint && (
            ...
            >
              ...
              >
                <Badge
                  variant="default"
                  className="bg-blue-600 text-[9px] sm:text-[10px] px-1.5 py-0.5"
                >
                  START POSITION
                </Badge>
            ...
            <PendingWaypointPopup
              ...
              onConfirm={selectedClimbStartTimeSecRemaining => {
                ...
                if (pendingWaypoint.type === 'climb') {
                  ...
                  const locationLabel =
                    climbLocation === 'side' ? 'Side' : climbLocation === 'middle' ? 'Middle' : '';
                  label =
                    `${locationLabel} ${climbResult === 'success' ? 'Succeeded' : 'Failed'}`.trim();
                } else {
                  delta =
