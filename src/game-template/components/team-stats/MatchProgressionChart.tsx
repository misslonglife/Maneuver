...
export function MatchProgressionChart({
  ...
  const primarySeriesKeyByEvent = new Map(
    primarySeries.map(series => [series.eventKey, series.key])
  );
  const compareSeriesKeyByEvent = new Map(
    compareSeries.map(series => [series.eventKey, series.key])
  );
  ...
  const chartConfig = allSeries.reduce<Record<string, { label: string; color: string }>>(
    (config, series) => {
      config[series.key] = {
        label: series.label,
        color: series.color,
