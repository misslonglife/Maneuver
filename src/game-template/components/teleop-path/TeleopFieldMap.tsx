...
function TeleopFieldMapContent({ scoutOptions }: { scoutOptions?: Record<string, boolean> }) {
  ...
  const [selectedDefenseEffectiveness, setSelectedDefenseEffectiveness] =
    useState<DefenseEffectiveness | null>(null);
  ...
  const addDefenseAction = useCallback(
    (
      zone: ZoneType,
      details?: {
  ...
  const startDefenseFlow = useCallback(
    (zone: ZoneType) => {
  ...
  const getSelectedDefenseTeamNumber = useCallback((): number | null => {
    const rawValue =
      customDefenseTeamInput.trim() !== '' ? customDefenseTeamInput : selectedDefenseTeam;
  ...
  const getTeleopHotkeyLabel = (elementKey: string): string | undefined => {
    ...
    if (elementKey === 'pass' || elementKey === 'pass_alliance' || elementKey === 'pass_opponent')
      return 'A';
    if (elementKey === 'tower') return 'F';
    if (
      elementKey === 'defense_alliance' ||
      elementKey === 'defense_neutral' ||
      elementKey === 'defense_opponent'
    )
      return 'D';
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (isDefenseTargetDialogOpen) {
        ...
        if (
          !isEditableTarget &&
          (key === 'n' || key === 'enter' || key === ' ' || key === 'spacebar')
        ) {
